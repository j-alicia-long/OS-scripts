...
void os_run(int initial_num_pages, page_table *pg_table){
	...
	// If page fault...
	// Track next empty frame index for bringing in new page
	// - Empty ones have 0 valid bit
	// Note: Frame index is for accessing RAM, but what about physical addr translation?
	// If no empty frames, use second-chance page replacement
	// - Store FIFO order in circular queue
	// Call disk_read
